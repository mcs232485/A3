...
def _monitor(dpp):
    while True:
        f=os.popen('ovs-ofctl dump-flows s1 -O Openflow13')
        flow=f.read().splitlines()
        tot = 0
        #print(flow)
        for n in flow:
            temp=n.split(",")
            ...
            if(len(temp)<9):
                ...
                if temp[7] == 'nw_src=10.0.0.3':
                    m=temp[3].split("=")
                 #   print("Traffic from Host 3")
                    tot=tot+int(m[1])
                  #  print(tot)
                    #print("mila")
                    #print(temp)
...
class FirewallController(ControllerBase):
    ...
    def regist_ofs(dp):
        ...
        f_ofs.set_enable_flow()
        f_ofs.set_arp_flow()
        f_ofs.set_log_enable()
        FirewallController._LOGGER.info('dpid=%s: Join as firewall.',
                                        dpid_str)
        url='http://localhost:8080/firewall/module/enable/'+dpid_str
        requests.put(url)
        obj={'nw_src': '10.0.0.1/32', 'nw_dst': '10.0.0.2/32'}
        url1='http://localhost:8080/firewall/rules/'+dpid_str
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.1/32', 'nw_dst': '10.0.0.3/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.1/32', 'nw_dst': '10.0.0.5/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.2/32', 'nw_dst': '10.0.0.1/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.2/32', 'nw_dst': '10.0.0.3/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.2/32', 'nw_dst': '10.0.0.4/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.3/32', 'nw_dst': '10.0.0.1/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.3/32', 'nw_dst': '10.0.0.2/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.3/32', 'nw_dst': '10.0.0.4/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.4/32', 'nw_dst': '10.0.0.2/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.4/32', 'nw_dst': '10.0.0.3/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.4/32', 'nw_dst': '10.0.0.5/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.5/32', 'nw_dst': '10.0.0.1/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.5/32', 'nw_dst': '10.0.0.4/32'}
        requests.post(url1, json=obj)
        obj={'nw_src': '10.0.0.3/32', 'nw_dst': '10.0.0.5/32', 'action': 'DENY'}
        requests.post(url1, json=obj)
        if dpid_str=='0000000000000001':
            x=threading.Thread(target = _monitor, args=(dpid_str,))
            x.start()
            
        #print(f_ofs.get_rules(None))
