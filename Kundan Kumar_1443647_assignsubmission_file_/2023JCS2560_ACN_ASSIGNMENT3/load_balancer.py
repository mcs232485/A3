...
class Load_balancer(app_manager.RyuApp):
    ...
    serv1_ip = '10.0.0.4'
    serv1_mac = '00:00:00:00:00:04'
    serv1_prt = 4
    serv2_ip = '10.0.0.5'
    serv2_mac = '00:00:00:00:00:05'
    serv2_prt = 4
    virt_ip = '10.0.0.42' 
    ...
    def _packet_in_handler(self, ev):
        ...
        if ev.msg.msg_len < ev.msg.total_len:
            self.logger.debug("We are truncating the packet : %s of %s bytes only",
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            if arp_header.dst_ip == self.virt_ip and arp_header.opcode == arp.ARP_REQUEST:
                self.logger.info("------------------------------")
                self.logger.info("------- ARP Packet is being handled ---------")
                ...
                self.logger.info("the ARP reply packet has been sent ")
                return
        if eth.ethertype == ETH_TYPE_IP:
            self.logger.info("------------------------------------")
            self.logger.info("---------------- We are Handling TCP Packet --------------")
    ...
    def generate_arp_reply(self, dst_ip, dst_mac):
        self.logger.info("We are Generating ARP Reply Packet")
        self.logger.info("ARP request ip address of client : " + dst_ip + ", client mac: " + dst_mac)
        ...
        if haddr_to_int(arp_target_mac) % 2 == 1:
            src_mac = self.serv1_mac
        else:
            src_mac = self.serv2_mac
        ...
        pkt.serialize()
        self.logger.info("Done after we have the ARP reply packet processed")
        return pkt

    def handle_tcp_packet(self, datapath, in_port, ip_header, parser, dst_mac, src_mac):
        packet_handled = False

        if ip_header.dst == self.virt_ip:
            if dst_mac == self.serv1_mac:
                server_dst_ip = self.serv1_ip
                server_out_port = self.serv1_prt
            else:
                server_dst_ip = self.serv2_ip
                server_out_port = self.serv2_prt

            # Route to server
            match = parser.OFPMatch(in_port=in_port, eth_type=ETH_TYPE_IP, ip_proto=ip_header.proto,
                                    ipv4_dst=self.virt_ip)
            ...
            match = parser.OFPMatch(in_port=server_out_port, eth_type=ETH_TYPE_IP,
                                    ...
                                    eth_dst=src_mac)
            actions = [parser.OFPActionSetField(ipv4_src=self.virt_ip),
